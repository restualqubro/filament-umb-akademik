...
class AktifResource extends Resource
{
        ...
        return $table
            ...
            ->actions([                
                Tables\Actions\Action::make('checked')
                    ...
                    })->visible(fn(Aktif $record) => $record->surat->status === 'Baru' && auth()->user()->roles->pluck('name')[0] === 'Operator'),
                Tables\Actions\Action::make('verifikasi')
                    ...
                    })->visible(fn (Aktif $record) => $record->surat->status === 'Checked' && auth()->user()->roles->pluck('name')[0] === 'Dosen'),
                Tables\Actions\Action::make('validasi')
                    ...
                    })->visible(fn (Aktif $record) => $record->surat->status === 'Verifikasi' && auth()->user()->roles->pluck('name')[0] === 'Admin'),
                Tables\Actions\Action::make('setuju')
                    ...
                    })->visible(fn (Aktif $record) => $record->surat->status === 'Validasi' && auth()->user()->roles->pluck('name')[0] === 'Wakil Rektor')                ,
                Tables\Actions\Action::make('print')
                    ...
                    ->openUrlInNewTab()                                                                                                
                    ->visible(fn (Aktif $record) => $record->surat->status === 'Disetujui' && auth()->user()->roles->pluck('name')[0] === 'Mahasiswa')                
            ])
            ->bulkActions([
    ...
    public static function getEloquentQuery(): Builder 
    {
        if (auth()->user()->roles->pluck('name')[0] === 'Dosen') {
            ...
            return Aktif::whereHas('surat.mahasiswa', function($q) {
