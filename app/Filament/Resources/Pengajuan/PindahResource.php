...
use App\Filament\Resources\Pengajuan\PindahResource\Pages;
use App\Filament\Resources\Pengajuan\PindahResource\RelationManagers;
...
class PindahResource extends Resource
{
    ...
    protected static ?string $pluralModelLabel = 'Pengajuan Pindah Perguruan Tinggi';
        ...
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('verifikasi')
                    ...
                    ->action(function (array $data, Pindah $surat): void {                                                                        
                        $record[] = array();                        
                    ...
                    })->visible(fn(Pindah $record) => $record->surat->status === 'Baru' || $record->surat->status === 'Diperbaiki' && auth()->user()->roles->pluck('name')[0] === 'Operator'),
                Tables\Actions\Action::make('validasi_dosen')
                    ...
                    ->action(function (array $data, Pindah $surat): void {                                                                        
                        ...
                        Pindah::where('id', $surat['id'])->update([
                            'dosen_id'      => $record['dosen_id']
                        ]);
                    })->visible(fn (Pindah $record) => $record->surat->status === 'Verifikasi' && auth()->user()->roles->pluck('name')[0] === 'Dosen'),
                Tables\Actions\Action::make('validasi_kaprodi')
                    ...
                    })->visible(fn (Pindah $record) => $record->surat->status === 'Validasi Dosen' && auth()->user()->roles->pluck('name')[0] === 'Kaprodi'),
                ...
                Tables\Actions\Action::make('disetujui')
                    ...
                    ->action(function (array $data, Pindah $surat): void {                                                                        
                        ...
                        $last = Pindah::whereHas('surat', function($q) {
                        ...
                        if ($last != null) {                                                                                            
                            ...
                            $code = "001/UM-BJM/S.3/".$monthRomanian."/".Carbon::now()->format('Y');
                        }                        
                        // $record['wrektor_id'] = auth()->user()->id;                        
                        // $record['status'] = "Disetujui";
                        Surat::where('id', $surat['surat_id'])->update([  
                            'update_detail' => 'Pengajuan anda telah mendapatkan Persetujuan dari Wakil Rektor 1 dan anda dapat menggunakan Surat Pengajuan Pindah ini sebagai Syarat Sah',                          
                            'status'        => 'Disetujui',                            
                        ]);
                        Pindah::where('id', $surat['id'])->update([
                    ...
                    })->visible(fn (Pindah $record) => $record->surat->status === 'Validasi Dekan' && auth()->user()->roles->pluck('name')[0] === 'Wakil Rektor'), 
                ...
                Tables\Actions\Action::make('ditolak_operator')
                    ...
                    })->visible(fn (Pindah $record) => $record->surat->operator_id === null && auth()->user()->roles->pluck('name')[0] === 'Operator'),
                Tables\Actions\Action::make('ditolak_dosen')
                    ...
                    ->requiresConfirmation()
                    ->action(function (array $data, Pindah $surat): void {                                                                                                
                        Pindah::where('surat_id', $surat['surat_id'])->update([                          
                            'status'        => 'Ditolak',                            
                        ]);                                              
                        Surat::where('id', $surat['surat_id'])->update([
                            'update_detail' => 'Pengajuan anda Ditolak oleh pihak Dosen Pembimbing Akademik',                          
                        ]);                        
                    })->visible(fn (Pindah $record) => $record->dosen_id === null && auth()->user()->roles->pluck('name')[0] === 'Dosen'),
                Tables\Actions\Action::make('ditolak_kaprodi')
                    ...
                    })->visible(fn (Pindah $record) => $record->kaprodi_id === null && auth()->user()->roles->pluck('name')[0] === 'Kaprodi'),
                Tables\Actions\Action::make('ditolak_dekan')
                    ...
                    })->visible(fn (Pindah $record) => $record->dekan_id === null && auth()->user()->roles->pluck('name')[0] === 'Dekan'),
                Tables\Actions\Action::make('ditolak_wrektor')
                    ...
                    })->visible(fn (Pindah $record) => $record->wrektor_id === null && auth()->user()->roles->pluck('name')[0] === 'Wakil Rektor'),
            ])
            ->bulkActions([
    ...
    public static function getEloquentQuery(): Builder 
    {
        if (auth()->user()->roles->pluck('name')[0] === 'Dosen') {
            return Pindah::whereHas('surat', function($q) {
                $q->where('status', '!=', 'Baru');
    ...
    {
        return [
            'index' => Pages\ListPindahs::route('/'),
            // 'create' => Pages\CreatePindah::route('/create'),
            // 'edit' => Pages\EditPindah::route('/{record}/edit'),
        ];
    }
}
