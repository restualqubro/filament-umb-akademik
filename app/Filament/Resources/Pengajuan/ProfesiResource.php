...
use App\Filament\Resources\Pengajuan\ProfesiResource\Pages;
use App\Filament\Resources\Pengajuan\ProfesiResource\RelationManagers;
...
class ProfesiResource extends Resource
{
    ...
    protected static ?string $pluralModelLabel = 'Pengajuan Tidak Lanjut Profesi';
        ...
        ->filters([
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('verifikasi')
                    ...
                    })->visible(fn(Profesi $record) => $record->surat->status === 'Baru' || $record->surat->status === 'Diperbaiki' && auth()->user()->roles->pluck('name')[0] === 'Operator'),
                Tables\Actions\Action::make('validasi_dosen')
                    ...
                    })->visible(fn (Profesi $record) => $record->surat->status === 'Verifikasi' && auth()->user()->roles->pluck('name')[0] === 'Dosen'),
                Tables\Actions\Action::make('validasi_kaprodi')
                    ...
                    })->visible(fn (Profesi $record) => $record->surat->status === 'Validasi Dosen' && auth()->user()->roles->pluck('name')[0] === 'Kaprodi'),
                ...
                Tables\Actions\Action::make('ditolak_operator')
                    ...
                    })->visible(fn (Profesi $record) => $record->surat->operator_id === null && auth()->user()->roles->pluck('name')[0] === 'Operator'),
                Tables\Actions\Action::make('ditolak_dosen')
                    ...
                    ->requiresConfirmation()
                    ->action(function (array $data, Profesi $surat): void {                                                                                                
                        Profesi::where('surat_id', $surat['surat_id'])->update([                          
                            'status'        => 'Ditolak',                            
                        ]);                                              
                        Surat::where('id', $surat['surat_id'])->update([
                            'update_detail' => 'Pengajuan anda Ditolak oleh pihak Dosen Pembimbing Akademik',                          
                        ]);                        
                    })->visible(fn (Profesi $record) => $record->dosen_id === null && auth()->user()->roles->pluck('name')[0] === 'Dosen'),
                Tables\Actions\Action::make('ditolak_kaprodi')
                    ...
                    })->visible(fn (Profesi $record) => $record->kaprodi_id === null && auth()->user()->roles->pluck('name')[0] === 'Kaprodi'),
                Tables\Actions\Action::make('ditolak_dekan')
                    ...
                    })->visible(fn (Profesi $record) => $record->dekan_id === null && auth()->user()->roles->pluck('name')[0] === 'Dekan'),
                Tables\Actions\Action::make('ditolak_wrektor')
                    ...
                    })->visible(fn (Profesi $record) => $record->wrektor_id === null && auth()->user()->roles->pluck('name')[0] === 'Wakil Rektor'),
            ])
            ->bulkActions([
    ...
    public static function getEloquentQuery(): Builder 
    {
        if (auth()->user()->roles->pluck('name')[0] === 'Dosen') {
            return Profesi::whereHas('surat', function($q) {
                $q->where('status', '!=', 'Baru');
