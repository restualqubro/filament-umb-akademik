...
class ProfesiResource extends Resource
{
        ...
        return $table
            ...
            ->actions([                
                Tables\Actions\Action::make('checked')
                    ...
                    })->visible(fn(Profesi $record) => $record->surat->status === 'Baru' && auth()->user()->roles->pluck('name')[0] === 'Operator'),
                Tables\Actions\Action::make('verifikasi')
                    ...
                    })->visible(fn (Profesi $record) => $record->surat->status === 'Checked' && auth()->user()->roles->pluck('name')[0] === 'Dosen'),
                Tables\Actions\Action::make('validasi')
                    ...
                    })->visible(fn (Profesi $record) => $record->surat->status === 'Verifikasi' && auth()->user()->roles->pluck('name')[0] === 'Admin'),
                Tables\Actions\Action::make('setuju')
                    ...
                    })->visible(fn (Profesi $record) => $record->surat->status === 'Validasi' && auth()->user()->roles->pluck('name')[0] === 'Wakil Rektor')                ,
                Tables\Actions\Action::make('print')
                    ->label('Print')
                    ->icon('heroicon-o-printer')
                    ->color('danger')                    
                    ->url(function(Profesi $record) {
                        return url('report/Profesi/'.$record->surat_id);
                    })
                    ->openUrlInNewTab()                                                                                                
                    ->visible(fn (Profesi $record) => $record->surat->status === 'Disetujui' && auth()->user()->roles->pluck('name')[0] === 'Mahasiswa')                
            ])
            ->bulkActions([
    ...
    public static function getEloquentQuery(): Builder 
    {
        if (auth()->user()->roles->pluck('name')[0] === 'Dosen') {
            ...
            return Profesi::whereHas('surat.mahasiswa', function($q) {
