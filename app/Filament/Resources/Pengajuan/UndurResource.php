        ...
        ->filters([
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('verifikasi')
                    ...
                    })->visible(fn(Undur $record) => $record->surat->status === 'Baru' || $record->surat->status === 'Diperbaiki' && auth()->user()->roles->pluck('name')[0] === 'Operator'),
                Tables\Actions\Action::make('validasi_dosen')
                    ...
                    })->visible(fn (Undur $record) => $record->surat->status === 'Verifikasi' && auth()->user()->roles->pluck('name')[0] === 'Dosen'),
                Tables\Actions\Action::make('validasi_kaprodi')
                    ...
                    })->visible(fn (Undur $record) => $record->surat->status === 'Validasi Dosen' && auth()->user()->roles->pluck('name')[0] === 'Kaprodi'),
                ...
                Tables\Actions\Action::make('disetujui')
                    ...
                    ->action(function (array $data, Undur $surat): void {                                                                        
                        ...
                        $last = Undur::whereHas('surat', function($q) {
                        ...
                        if ($last != null) {                                                                                            
                            ...
                            $code = "001/UM-BJM/S.4/".$monthRomanian."/".Carbon::now()->format('Y');
                        }                        
                        // $record['wrektor_id'] = auth()->user()->id;                        
                        // $record['status'] = "Disetujui";
                        Surat::where('id', $surat['surat_id'])->update([  
                            'update_detail' => 'Pengajuan anda telah mendapatkan Persetujuan dari Wakil Rektor 1 dan anda dapat menggunakan Surat Pengajuan Mengundurkan Diri ini sebagai Syarat Sah',                          
                            'status'        => 'Disetujui',                            
                        ]);
                        Undur::where('id', $surat['id'])->update([
                    ...
                    })->visible(fn (Undur $record) => $record->surat->status === 'Validasi Dekan' && auth()->user()->roles->pluck('name')[0] === 'Wakil Rektor'), 
                ...
                Tables\Actions\Action::make('ditolak_operator')
                    ...
                    })->visible(fn (Undur $record) => $record->surat->operator_id === null && auth()->user()->roles->pluck('name')[0] === 'Operator'),
                Tables\Actions\Action::make('ditolak_dosen')
                    ...
                    ->requiresConfirmation()
                    ->action(function (array $data, Undur $surat): void {                                                                                                
                        Undur::where('surat_id', $surat['surat_id'])->update([                          
                            'status'        => 'Ditolak',                            
                        ]);                                              
                        Surat::where('id', $surat['surat_id'])->update([
                            'update_detail' => 'Pengajuan anda Ditolak oleh pihak Dosen Pembimbing Akademik',                          
                        ]);                        
                    })->visible(fn (Undur $record) => $record->dosen_id === null && auth()->user()->roles->pluck('name')[0] === 'Dosen'),
                Tables\Actions\Action::make('ditolak_kaprodi')
                    ...
                    })->visible(fn (Undur $record) => $record->kaprodi_id === null && auth()->user()->roles->pluck('name')[0] === 'Kaprodi'),
                Tables\Actions\Action::make('ditolak_dekan')
                    ...
                    })->visible(fn (Undur $record) => $record->dekan_id === null && auth()->user()->roles->pluck('name')[0] === 'Dekan'),
                Tables\Actions\Action::make('ditolak_wrektor')
                    ...
                    })->visible(fn (Undur $record) => $record->wrektor_id === null && auth()->user()->roles->pluck('name')[0] === 'Wakil Rektor'),
            ])
            ->bulkActions([
